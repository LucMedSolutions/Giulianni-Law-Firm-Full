...
import { AlertCircle, Search, Filter, ArrowUpDown, FileText, Download, ExternalLink } from "lucide-react"
...
export default function DatabaseManagement() {
  ...
  const filteredCases = cases
    .filter((caseItem) => {
      ...
      // Apply status filter
      const matchesFilter = caseFilter === "all" || caseItem.status === caseFilter
  ...
  const filteredDocuments = documents
    .filter((doc) => {
      ...
      const matchesSearch =
        ...
        (doc.cases && doc.cases.case_number?.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  if (loading) {
    return <div className="text-center py-10">Loading database information...</div>
  }
                      ...
                      filteredCases.map((caseItem) => {
                        ...
                        return (
                          <tr key={caseItem.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{caseItem.case_type}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      filteredUsers.map((user) => (
                        <tr key={user.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            ...
                            >
                              {user.role}
                              {user.role === "staff" && user.staff_role && ` (${user.staff_role})`}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
